# <div class="fi fi-it"></div> Pielle

## Infos

### Data
* **Origin:** Italy
* **Defense:** 118
* **Stun:** 2896

### Wrong origin ?
The flag displayed on his stage was changed from France to Italy between Breakers and Breakers Revenge. In Breakers Chicago, his name has changed to Pierre, seemingly correcting Pielle, which appears to have been a phonetic misspelling of the French name.

### Story

Pielle is an Italian nobleman and skilled fencer who can summon lightning with his sword. Georges, his French alternate, despises Italians after being robbed.

### Colors

| [A] | [B] | [C] | [D] |
|:----------:|:----------:|:----------:|:----------:|
| ![img](media/characters/pielle/img/pielle_A.png)  | ![img](media/characters/pielle/img/pielle_B.png)  | ![img](media/characters/pielle/img/pielle_C.png)  | ![img](media/characters/pielle/img/pielle_D.png)  |


## Moves

### Normals
All normals
> [A] far/close : [MID] [SPECIAL] [SUPER]
> [A] down : [MID] [SPECIAL] [SUPER]
> [A] jump : [HIGH]
> [B] far/close : [MID] [SPECIAL] [SUPER]
> [B] down : [LOW] [SPECIAL] [SUPER]
> [B] jump : [HIGH]
> [C] far : [MID]
> [C] close : [MID] [SPECIAL] [SUPER]
> [C] down : [MID] [SPECIAL] [SUPER]
> [C] jump : [HIGH]
> [D] far : [MID]
> [D] close : [MID] [SPECIAL] [SUPER]
> [D] down : [LOW]
> [D] jump : [HIGH]

### Commands
Saber Bash | [A]+[C]
> <img src="/media/characters/pielle/img/moves/SaberBash.gif">
> [MID]
> - Not overhead, despite the animation.
> - Unsafe on block and on hit at close range.
> - Best used as a long-range poke.
Flying Peach Bomber | [B]+[D]
> <img src="/media/characters/pielle/img/moves/FlyingPeachBomber.gif">
> [HIGH]
> - Quick overhead
> - Safe on block.
> - Hit confirm with 5A
> - Leaves a punishable gap when canceled from normals
Hopping Saber | [AIR],[DOWN]+[A]
> <img src="/media/characters/pielle/img/moves/HoppingSaber.gif">
> [HIGH]
> - Stomp attack
> - Can be repeated based on range and height.
> - Follow with down+A+B+C while in the air.
Opened Arc Liner | [AIR],[UP]+[C]
> <img src="/media/characters/pielle/img/moves/OpenedArcLiner.gif">
> [HIGH]
> - Provides more range than a regular jump C.

### Specials
Bonjour Shot | [QCF]+[P]
> <img src="/media/characters/pielle/img/moves/BonjourShot.gif">
> [MID]
> - Projectile
> - Has a slow recovery on hit and block.
Robin Liner | [DP]+[P]
> <img src="/media/characters/pielle/img/moves/RobinLiner.gif">
> [MID] 
> - Lacks invincibility and has high recovery.
> - Light version is fast but doesn't knockdown.
> - Heavy version does knockdown.
Monsieur Crash | [QCB]+[P].[P].[P]
> <img src="/media/characters/pielle/img/moves/MonsieurCrash.gif">
> [MID]
> - Chains up to 3 times with repeated punch presses.
> - All hits are unsafe on block.
> - Can be delayed to counter punish attempts.
Rose Train | [QCB]+[K]
> <img src="/media/characters/pielle/img/moves/RoseTrain.gif">
> [MID]
> - Pushes the opponent toward the corner on hit.
> - Only confirms on standing opponents.
> - For crouching opponents use 2C (launcher).
Merci Un Deux Trois | [AIR],[DOWN]+[A]+[B]+[C]<br>[AIR],[DOWN]+[B]+[C]+[D]
> <img src="/media/characters/pielle/img/moves/MerciUnDeuxTrois.gif">
> [HIGH]
> - Aerial dive used to bait anti-airs or jumps.
> - Can be canceled from air normals.
> - Unsafe on block.

### Supers
Un Deux Robin | [QCF],[QCF]+[P]
> <img src="/media/characters/pielle/img/moves/UnDeuxRobin.gif">
> [MID] [REVERSAL] [HKD] 
> - Passes through projectiles.
> - Hits OTG (Off The Ground) after an anti-air slide.
Thunder Croiser | [QCB],[QCB]+[P]
> <img src="/media/characters/pielle/img/moves/ThunderCroiser.gif">
> [MID] [HKD] 
> - anti-air with invincible startup and high reach
Super Rose | [QCB],[QCB]+[K]
> <img src="/media/characters/pielle/img/moves/SuperRose.gif">
> [MID] [REVERSAL] [HKD] 
> - Multi-hit, anti-air. and full-screen Super with a massive hitbox.
> - Safe on block.



## Guide


### Overview
Pielle is effective up close thanks to his quick overhead, but he’s weak defensively without Super. He’s not easy to use, since he lacks low confirms and invincible reversals, and his specials recover slowly, making them risky and easy to punish.

### Neutral
<video>
    <source data-src="media/characters/pielle/videos/pielle_neutral.mp4">
</video>

* Use multiples 5A or 2A as pokes. Hit confirm with 214P or 236236P Super. 
* You can also follow with B+D (overhead) to find openings.
* His Special has high recovery; use them with caution to avoid punishes.
* 623A doesn’t knock down grounded opponents. Use 623C for combo extensions.


### Anti-air

<video>
  <source data-src="media/characters/pielle/videos/pielle_anti-air.mp4">
</video>


* Close 5C: Effective anti-air/anti-crossup; cancelable into specials or supers.
* Thunder Croiser (214214P): Invincible anti-air with long (vertical) reach.
* 623A: Reliable option for a quick anti-air.


### Super moves
<video>
  <source data-src="media/characters/pielle/videos/matchups/pielle_vs_dao-long_kouryuu_ga.mp4">
</video>

* Robin Super (236236P): Pass-through projectile; hits OTG after an anti-air slide.
* Thunder Croiser (214214P): Use it as a reversal or anti-air.
* Super Rose (214214K): Safe on block with full-screen reach; also functions as an anti-air.

### Tips
#### Anti-air slide (2D)
<video>
    <source data-src="media/characters/pielle/videos/pielle_slide_otg.mp4">
</video>

* His slide can be used as an anti-air, leading to an OTG with Super 236236P or 214214P (corner).


#### Flying Peach Bomber (overhead)
<video>
    <source data-src="media/characters/pielle/videos/rose_train.mp4">
</video>

* Most of Pielle’s normals cancel into his overhead, 
* but there’s a gap. On hit, confirm with 5A or 5B. 
* It whiffs on crouch and makes him pass through.


#### Fuzzy
<video>
  <source data-src="media/characters/pielle/videos/fuzzyguard.mp4">
</video>

* You can perform a fuzzy using jC into jB, jC or jD.
* jD can be canceled with down+A+B+C, but it doesn’t combo.

### Replays
* [<div class="fi fi-br"></div>lucasrocha2013 vs  <div class="fi fi-br"></div>Carecation](https://youtu.be/Vjqd19gWLO8?si=bwCqriEpvHoRBhAO)
* [<div class="fi fi-br"></div>lucasrocha2013 vs  <div class="fi fi-br"></div>Markadv](https://www.youtube.com/watch?v=weAUQFh83x4)
* [<div class="fi fi-us"></div>MorkmanFreeman vs  <div class="fi fi-br"></div>lucasrocha2013](https://youtu.be/Q8bSknwe7so?si=hMFp-cN9TZxtYXcW)
* [<div class="fi fi-us"></div>SouthpawRock vs  <div class="fi fi-br"></div>lucasrocha2013](https://youtu.be/Q8bSknwe7so?si=hMFp-cN9TZxtYXcW)


### Ressources   
* [SuperCombo Wiki - Breakers Revenge/Pielle](https://wiki.supercombo.gg/w/Breakers_Revenge/Pielle)


## Combos

### Meterless combos

<video>
<source data-src="media/characters/pielle/videos/meterless_combo.mp4">
</video>

* 2B > 214C.C.C
* 2A, 2A, 2A > 214C.C.C
* jD > 2C > 214D
* jD > 2C > 214C.C.C


### Metered combos
<video>
<source data-src="media/characters/pielle/videos/metercombo.mp4">
</video>

* 2A, 2A > Super Swords (strict timing)
* jD > close 5D > Super Swords
* jD > 2C > Super Swords
* corner: jD > 2C > A+C > Super Swords / Thunder


### Anti-air
<video>
<source data-src="media/characters/pielle/videos/antiair_combo.mp4">
</video>

* close 5D > Super Swords / Thunder
* close 5C > Super Swords
* close 5C > Super Thunder (corner)
* 2C > 214B or 214D
* 2C > Super Flower
* 2C > Super thunder (close range)
* corner: 2C > 214B > Super Swords / Thunder
* far. 5D > Swords
* slide > Super Swords / Thunder (OTG)


### Air-to-air
<video>
<source data-src="media/characters/pielle/videos/airtoair.mp4">
</video>

* jB/HK/HP/A > Super
* jB/HK/HP/A > 623A (1st hit must be close to the ground)


### Rose train
<video>
<source data-src="media/characters/pielle/videos/rose_train.mp4">
</video>

Only on standing opponents:
* B+D ❌ 5A x2 > 214B
* 2A > close 5A > 214B
* 5B > 214B
* 2B > 214B

Close range :
* 2C > 214B or 214D


### Breakable combos

* B+D ❌ 5B
* B+D ❌ 2B
* B+D ❌ 5A


## Matchups

### Sho

#### Ranki Ryuu Geri
<video>
<source data-src="media/characters/pielle/videos/matchups/pielle_vs_sho_ranki_ryuu_geri.mp4">
</video>

- Unsafe, but Sho can cancel into Air Super to stay safe and bait punish attempts. 
- If you have meter, use Super Rose (214214K) for a guaranteed punish.

### Condor

#### Atlas Rock Buster
<video>
    <source data-src="media/characters/pielle/videos/matchups/pielle_vs_condor_atlas_rock_buster.mp4">
</video>

- There’s a gap after the 1st hit, punish with throw or Thunder Croiser (214214P)

#### Gran Buffalo Horn
<video>
    <source data-src="media/characters/pielle/videos/matchups/pielle_vs_condor_slide.mp4">
</video>

- Unsafe on block. Punish with 2C

#### Tomahawk Jack
<video>
    <source data-src="media/characters/pielle/videos/matchups/pielle_vs_condor_tomahawk.mp4">
</video>

- Unsafe on block. Punish with 2C
- Whiff punish with Super Rose (214214K)

### Rila

#### Killer Snake
<video>
  <source data-src="media/characters/pielle/videos/matchups/pielle_vs_rila_killer_snake.mp4">
</video>

- Unsafe. Heavy version can be followed by any super to bait punishes.
- Punish with 2A, it hits if she does nothing
- In case she does a Super, counter it with Thunder Croiser (214214P)

#### Shine Beast
<video>
  <source data-src="media/characters/pielle/videos/matchups/pielle_vs_rila_shine_beast.mp4">
</video>

- On block, punish with Super Rose (214214K)



### Tia

#### Soul Spike
<video>
  <source data-src="media/characters/pielle/videos/matchups/pielle_vs_tia_soul_spike.mp4">
</video>

- Not overhead
- Punish light version with super (236236P)
- Heavy version can be followed with super to bait your punish attempt. Use throw to avoid it.

### Alsion

#### Deadly Claw
<video>
  <source data-src="media/characters/pielle/videos/matchups/pielle_vs_alsion_deadly_claw.mp4">
</video>
    
- At close range, 214B stays in front, while 214D lands behind. 
- Whiff punish with 2C.


### Maherl

#### Cadabra Bomber
<video>
  <source data-src="media/characters/pielle/videos/matchups/pielle_vs_maherl_cadabra_bomber.mp4">
</video>

- invincible startup and Safe on block
- If close, punish after blocking the 1st hit with super rose (214214K)

### dao-long

#### Kouryuu Ga
<video>
  <source data-src="media/characters/pielle/videos/matchups/pielle_vs_dao-long_kouryuu_ga.mp4">
</video>

- In corner (close range) punish with 2A
- Whiff punish with 236236P (requires spacing).

### Saizo

#### Yashagami
<video>
  <source data-src="media/characters/pielle/videos/matchups/pielle_vs_saizo_yashagami.mp4">
</video>

- Unsafe on block and hit unless if canceled into super.
- They may delay a super on block, but 236236P still guarantees a punish.

